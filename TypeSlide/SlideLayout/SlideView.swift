...
//			}
			...
			.onAppear {
				...
				//let colors = generateColorGradient(startRGB: (74, 165, 248), endRGB: (244, 179, 63), numElements: 10)
				// let colors = generateColorChanges(startRGB: (74, 165, 248), endRGB: (244, 179, 63), numElements: 10)
				//let colors = generateColors(n: 100)


				let colors = createColorGradient(sampleSize: 100)
				let color = colors[60]

//				let rgb = generateRgbColorsFunky(n: 10)[2]
//				let rgb = generateRgbColorsFunkyOrange(n: 10)[4]
				let rgb = generateRgbColorsFunkyOrangePink(n: 33)[11]

//				let rgb = (color[0], color[1], color[2])
				print(rgb)
				backgroundColor = Color(.displayP3, red: Double(rgb.0) / 255, green: Double(rgb.1) / 255, blue: Double(rgb.2) / 255)
			}
		}
		.shadow(radius: 0.01)
	}
